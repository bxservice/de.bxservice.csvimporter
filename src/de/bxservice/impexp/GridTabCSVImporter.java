 ...
 * 
 * @Improved to work faster with asynchronous commits
 *        - Bx Service GmbH
 *        - Thomas Bayen
 *        - Diego Ruiz
 */
								   ...
								   if(isThereDocAction){ 
									  ...
									  if(isError){
									   ...
									   if( DB.isPostgreSQL() && isAsynchronousCommit() ){
										   Statement stm = trx.getConnection().createStatement();
										   stm.execute("set local synchronous_commit = off");
										   stm.close();
									   }
									   trx.commit();  
									   for(String row:rowsTmpResult)						   
...
//				}		
    ...
    public boolean isAsynchronousCommit() {
		return isAsynchronousCommit;
	}

	public void setAsynchronousCommit(boolean isAsynchronousCommit) {
		this.isAsynchronousCommit = isAsynchronousCommit;
	}
